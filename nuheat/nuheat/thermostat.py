...
from .config import SCHEDULE_RUN, SCHEDULE_TEMPORARY_HOLD, SCHEDULE_HOLD
from .util import (
    celsius_to_nuheat,
...
class NuHeatThermostat(object):
    ...
    def schedule_mode(self, mode):
        ...
        :param mode: The desired mode integer value.
                     ...
                     Permanent hold = 3
        """
        modes = [SCHEDULE_RUN, SCHEDULE_TEMPORARY_HOLD, SCHEDULE_HOLD]
        ...
        if mode == SCHEDULE_HOLD:
            post_data["ManualTemperature"] = self.manual_temperature
    ...
    def hold_time(self):
        ...
        """
        if self._schedule_mode == SCHEDULE_TEMPORARY_HOLD:
    ...
    def hold_time(self, hold):
        ...
        post_data = {
            "RegulationMode": SCHEDULE_TEMPORARY_HOLD,
    ...
    def resume_schedule(self):
        ...
        """
        self.schedule_mode = SCHEDULE_RUN

    def set_target_fahrenheit(self, fahrenheit, mode=SCHEDULE_HOLD, hold_time=None):
        """
    ...
    def set_target_celsius(self, celsius, mode=SCHEDULE_HOLD, hold_time=None):
        """
    ...
    def set_target_temperature(self, temperature, mode=SCHEDULE_HOLD, hold_time=None):
        """
        ...
        modes = [SCHEDULE_TEMPORARY_HOLD, SCHEDULE_HOLD]
        if mode not in modes:
            raise Exception("Invalid mode. Please use one of: {}".format(modes))
        ...
        if mode == SCHEDULE_HOLD:
            post_data["ManualTemperature"] = temperature

        if mode == SCHEDULE_TEMPORARY_HOLD:
    ...
    def set_data(self, post_data):
        ...
        """
        _LOGGER.debug(f"Setting Data: Url{self._url} Payload {post_data}")
        params = {
            "serialnumber": self.serial_number
        }
